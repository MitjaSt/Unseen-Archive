...
export default function Add() {
  ...
  const typeOptions = {
    link: { label: 'Link', value: ItemType.LINK },
    note: { label: 'Note', value: ItemType.NOTE }
  };

  const [type, setType] = useState<ItemType>(typeOptions.link.value);
  ...
  const validateAndSubmit = (navigateToList: boolean) => {
    ...
    const result = itemSchema.safeParse({
      type,
      link: type === ItemType.LINK ? link : undefined,
      note: type === ItemType.NOTE ? note : undefined,
    });
